...
if ! bashio::fs.file_exists "${mount_script}"; then
  ...
  #
  # Per-device exponential backoff on the attach attempt itself (2026-08-26):
  # during a real outage the server/network side stays down for a while, and
  # hammering `usbip attach` again every single 15s compounds an already
  # unstable connection instead of giving it room to recover. The `usbip
  # port` status check stays on the full 15s cadence regardless (cheap,
  # non-disruptive, and needed to promptly notice a fresh drop on any
  # device) - only the disruptive attach command backs off, and only for
  # the specific device that's failing; other devices keep their own
  # independent cadence. Backoff resets to the base interval the moment a
  # device is seen attached again.
