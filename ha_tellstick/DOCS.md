...
tellstick:
    host: <repository_hash>-ha-tellstick
    port: [50800, 50801]
```
...
call `hassio.addon_stdin` with the add-on's real slug (not just `tellstick` -
third-party repo add-ons are namespaced as `<repository_hash>_ha_tellstick`,
visible in Settings -> Add-ons -> TellStick -> the URL or the three-dot menu):

```json
{"addon": "<repository_hash>_ha_tellstick", "input": {"function": "learn", "device": "1"}}
```
...
add-on log: `{"addon": "<repository_hash>_ha_tellstick", "input": {"function": "list-sensors"}}`
...
Upstream is unmaintained, so please don't file issues there for anything
related to this fork. Open an issue on [pallemannen/hass-addons][repo] instead.
...
[repo]: https://github.com/pallemannen/hass-addons
